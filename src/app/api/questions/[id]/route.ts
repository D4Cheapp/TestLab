import { questionCreateRequestType, requestTypesType } from "@/src/types/requestTypes";
...
export async function DELETE(req: NextRequest, { params }: { params: { id: number } }) {
  try {
    ...
    const data = await createFetch<questionCreateRequestType>({
...
export async function PATCH(req: NextRequest, { params }: { params: { id: number } }) {
  try {
    const cookie = cookies().get('_session_id');
    const reqBody = await Promise.resolve<questionCreateRequestType>(req.json());
